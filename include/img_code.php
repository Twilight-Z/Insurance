...
function get_str($length = 4){
	...
	// $codestr = substr($tmp,0,$length);//对打乱的字符串进行截取

	$arr1 = range('a', 'z');
   $arr2 = range('A', 'Z');
   $arr3 = range(0, 9);
   $codearr = array_merge($arr1, $arr3, $arr2, $arr3);/* 合并数组 */
   shuffle($codearr);/* 打乱数组 */
	$codestr = implode('', array_slice($codearr, 0, $length));
...
$width = 90;//定义图片宽度
$height = 34;//定义图片高度
$strNum = get_str($length);//获取一个随机字符串
